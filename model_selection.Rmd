...
select(mental_health = MCMH_MENTAL_HISTORY, num_arrests = V1197, sentence_length = CS_SENTENCEMTH, age = V0014, race = V2982, sex = V0005, abuse = SES_PHYSSEXABUSED_EVER, hiv = MCMH_RSLT_HIV) %>%
  mutate(mental_health = recode(mental_health, "(0000001) Yes" = "Yes", "(0000002) No" = "No"),
         abuse = recode(abuse, "(0000004) Not abused" = "Not Abused", "(0000003) Both physically and sexually abused" = "Physical & Sexual abuse", "(0000002) Sexually only" = "Sexual only", "(9999999) Missing" = "Missing",  "(0000001) Physically only" = "Physical only"))%>%
  filter(mental_health %in% c("Yes", "No"))

full_model =
  glm(relevel(mental_health, ref = "No") ~ num_arrests + sentence_length + age + race + sex + relevel(abuse, ref = "Not Abused") + relevel(hiv, "Presumed Negative"), data = model_data, family = "binomial")
...
manual_df =
model_data %>%
  filter(abuse %in% c("Not Abused", "Physical & Sexual abuse"), race != "Multiple Races reported, non-Hispanic")

manual_model = 
  glm(relevel(mental_health, ref = "No") ~ num_arrests + age + race + sex + relevel(abuse, ref = "Not Abused") + relevel(hiv, "Presumed Negative"), data = manual_df, family = "binomial")
...
manual_model2 = 
  glm(relevel(mental_health, ref = "No") ~ num_arrests + race + sex + relevel(abuse, ref = "Not Abused") + relevel(hiv, "Presumed Negative"), data = manual2, family = "binomial")
