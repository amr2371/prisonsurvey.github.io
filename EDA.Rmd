...
Subsequently, we stacked these two datasets using a newly created variable called `dataset` to identify which original file each observation was from (i.e., state vs. federally incarcerated). We also cleaned the resulting data by checking for coding errors, recoding various factor variables, adding variable labels, and changing the coding of values from `missing`/`don't know`/`unknown` to `NA` for consistency.
...
From this table, we can see that certain variables appear to differ by mental health history, one of our primary outcomes of interest. Therefore, the statistical analysis page of the website also includes hypothesis testing of some of these variables to assess whether the observed differences are significant.  
...
Subsequently, a table was created to display the distribution of variables related to individuals' current incarceration and/or history regarding the justice system. These variables included number of prior sentences (`CH_PRIORSENTENCE_NUM`), number of prior arrests (`V1197`), collapsed criminal history (`CH_CRIMHIST_COLLAPSED`), whether their parents have been incarcerated (`SES_PARENTS_INCARCERATED`), whether their other immediate family has been incarcerated (`SES_FAMILY_INCARCERATED`), and whether they are incarcerated in a state or federal facility (`dataset`).
...
Again, some of these variables appear to differ by mental health history status, so some hypothesis testing is shown on the Statistical Analysis page of the website, which examines this further.  
...
hosp_year = 
  ...
  labs(x = "Hospitalized for mental health, year before arrest") 
