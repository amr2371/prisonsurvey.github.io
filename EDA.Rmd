...
total_df %>% 
  group_by(TYPEOFFENSE, MENTAL_HISTORY_BINARY) %>% 
  summarize(count = n()) %>% 
   mutate(
    TYPEOFFENSE = gsub(".*)", "", TYPEOFFENSE)) %>% 
  rename(MH_History = MENTAL_HISTORY_BINARY) %>%
  ungroup() %>% 
  ggplot(aes(x = TYPEOFFENSE, y = count, fill = MH_History)) +
...
```{r mental_health, message=FALSE}
#Ever taken a medication for mental health 

# federal_df %>% 
#   group_by(V2409) %>% 
#   summarize(count = n()) %>% 
#   rename('Ever taken a mental health medication' = V2409) %>%
#   knitr::kable()

total_df %>% 
  filter(V2409 == "(1) Yes" | V2409 == "(2) No" | V2409 == "(7) Don't know") %>%
  group_by(V2409, dataset) %>% 
  summarize(count = n()) %>% 
   mutate(
    V2409 = fct_reorder(V2409, count)) %>% 
  ungroup() %>% 
  ggplot(aes(x = V2409, y = count, fill = dataset)) +
  geom_bar(position = "dodge", stat = "identity") + 
...
total_df %>% 
  group_by(V2412, dataset) %>% 
  summarize(count = n()) %>% 
  drop_na() %>% 
   mutate(
    V2412 = fct_reorder(V2412, count)) %>% 
  ungroup() %>% 
  ggplot(aes(x = V2412, y = count, fill = dataset)) +
  geom_bar(position = "dodge", stat = "identity") + 
  theme(legend.title=element_blank()) +
...
total_df %>% 
  ...
  drop_na() %>% 
   mutate(
    MCMH_MHOSPYR = fct_reorder(MCMH_MHOSPYR, count),
    MCMH_MHOSPYR = str_sub(MCMH_MHOSPYR, 10)
    ) %>% 
  ungroup() %>% 
  ggplot(aes(x = MCMH_MHOSPYR, y = count, fill = dataset)) +
  geom_bar(position = "dodge", stat = "identity") + 
  theme(axis.text.x = element_text(angle=45, hjust=1), legend.title=element_blank()) +
  labs(x = "Hospitalized for mental health, in the year before arrest") 
...
total_df %>% 
  group_by(MCMH_MHOSPADM, dataset) %>% 
  summarize(count = n()) %>% 
    filter(!MCMH_MHOSPADM %in% c("(9999999) Missing","(9999998) DK/refused")) %>% 
  drop_na() %>% 
   mutate(
    MCMH_MHOSPADM = fct_reorder(MCMH_MHOSPADM, count),
    MCMH_MHOSPADM = str_sub(MCMH_MHOSPADM, 10)) %>% 
  ungroup() %>% 
  ggplot(aes(x = MCMH_MHOSPADM, y = count, fill = dataset)) +
  geom_bar(position = "dodge", stat = "identity") + 
  theme(axis.text.x = element_text(angle=45, hjust=1), legend.title=element_blank()) +
...
# HIV 
```{r hiv, messgae=FALSE}
table1(~ MCMH_RSLT_HIV | dataset, data = total_df, overall = "Total")
```


# HIV & Mental Health

```{r hiv mh, message=FALSE}
