...
To assess whether mental health (outcome of interest) is associated with HIV status (exposure of interest), we conducted logistic regression modeling (due to the binary nature of the outcome variable). First, we created a crude model of this association with no additional covariates.
...
* Null hypothesis: Mental health history (yes vs. no history of at least one mental health condition) does not differ by number of prior arrests (measured as continuous).

* Alternate hypothesis: Mental health history (yes vs. no history of at least one mental health condition) DOES differ by number of prior arrests (measured as continuous).
